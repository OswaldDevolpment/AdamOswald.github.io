...
async function main() {
  ...
  else {console.log('You entered an invalid type, please try again.')
        return main()
    }
    async function rip() {
    if (type == '.rbxm' || type == '.mp3' || type == '.webm' ||
        type == ".rbxl") {
      response = await download(
          asset, {method : 'GET', headers : {'User-Agent' : 'Roblox/WinInet'}});
      ...
      result1 = response.split(
          /<Content name="TextureId"><url>http:\/\/www.roblox.com\/asset\/\?id=([0-9]+)/)
      result2 =
          /<Content name="MeshId"><url>http:\/\/www.roblox.com\/asset\/\?id=([0-9]+)/
              .exec(response)
...
async function download(asset, headers) {
        return await fetch(
            `https://assetdelivery.roblox.com/v1/asset?id=${asset}`,
            headers ? headers : {})
}