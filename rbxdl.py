...
def saveAsset(astId, astTypeStr, cDir, sDirName, astData, astVer):
    try:
        ...
        saveLocation = createDirectory(
            f'{cDir}\\{astTypeStr}\\{astId}') if sDirName is True else f'{cDir}\\{astTypeStr}'
...
def download(astId, astVer, args):
    ...
    if resp[0] == 200:
        print(f'Saving: {url}...')
        save = saveAsset(astId, astTypes[getMeta(
            astId, 'AssetTypeId')][0], cDir, sDir, resp[1].content, astVer)
        if save == 1:
...
def handleArgs(args):
    ...
    if dlm == 'single':
        ...
        if isinstance(astId, list):
            ...
            raise ValueError(
                'Incorrect format for bulk downloading. Should be laid out as [id1,id2,id3,etc..]')
    elif dlm == 'range':
        if isinstance(astId, list) and len(astId) == 2:
            ...
            raise ValueError(
                'Incorrect format for range downloading. Should be laid out as [minId, maxId]')
    elif dlm == 'roulette':
...
cmdParse.add_argument('downlmode', choices=[
                      'single', 'bulk', 'range', 'roulette'], help='mode for asset downloading', type=str)
cmdParse.add_argument('assetid', help='id(s) of asset', type=str)
cmdParse.add_argument(
    '--dir', help='save assets into your own directory', type=str)
...
cmdParse.add_argument(
    '--allVer', help='Download all of the versions of an asset (slow)', action='store_true')
cmdParse.add_argument(
